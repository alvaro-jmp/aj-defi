...
exports.get_app = function (ref_fb, ref_fb_admin, admin_firestore, admin_real_time_db) {
  ...
  var c_home = require('./c_home');

  var c_login = require('./c_login'); // const t = require('./lib/tools')
  // const routes_home = [/^(|\/)$/]
  // const routes_dashboard = [/^\/cnp\/dashboard(|\/)$/, /^\/cnp(|\/)$/]


  var c_name = 'core -->'; // controller name

  app.use(cookieParser());
  app.use(helmet()); // app.use(routes_home, (req, res, next) => {
  ...
  app.use(c_login.get_router(ref_fb, ref_fb_admin, p_get_secret, app, admin_firestore, admin_real_time_db)); // app.use(p_view_dashboard.fun(ref_fb, ref_fb_admin, p_get_secret, admin_firestore))  
